...
void setup()
{
  ...
  *  problem with strings(ie. array of char) since every space is filled with zeros.
  ...
  Serial.print("SRAM Space after calloc(heap_int): ");
  Serial.println(getSRAM());
  Serial.println();
...
//  Serial.println();
  ...
  */
  free(heap_lines);
  /*
   * Proper free order is always the key. DO NOT free a pointer again if you have freed
   * already! (ie. below is a no-no)
  */
  free(heap_string);
